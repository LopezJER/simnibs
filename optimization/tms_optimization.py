...
def _create_grid_modified(
    mesh, pos, distance, radius, resolution_pos, scalp_normals_smoothing_steps=20, scalp_boundary_z=None
):
    """Creates a position grid"""
    # extract ROI
    msh_surf = mesh.crop_mesh(elm_type=2)
    ...
    if scalp_boundary_z is not None:
        inside = inside & (coords_mapped[:, 2] >= scalp_boundary_z)
    return coords_mapped[inside], coords_normals[inside]
...
def get_opt_grid_modified(
    ...
    """
    # creates the spatial grid
    coords_mapped, coords_normals = _create_grid_modified(
        mesh, pos, distance, radius, resolution_pos, scalp_normals_smoothing_steps, scalp_boundary_z
    )
