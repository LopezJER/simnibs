...
class POSITION(object):
    ...
    def get_scalp_mask_nodes(self, msh, cap, plane_y, retreat_mm=0.0):
        """Finds 1-indexed SimNIBS Node IDs inside the electrode rim boundary.
        Uses element centroids for filtering to avoid jagged boundary artefacts.
        retreat_mm shrinks the mask upward from the frontal/temporal rim arc,
        leaving the posterior plane_y boundary unchanged.
        retreat_mm=0 keeps the full mask.
        """
        from scipy.spatial import cKDTree
        print(1234567, retreat_mm)
        # 1. Isolate scalp triangles and their nodes (Tag 1005)
        scalp_tri_mask = (msh.elm.elm_type == 2) & (msh.elm.tag1 == 1005)
        all_tris       = msh.elm.node_number_list[scalp_tri_mask, :3]  # 1-indexed
        all_scalp_ids  = np.unique(all_tris)
        scalp_coords   = msh.nodes.node_coord[all_scalp_ids - 1]
        tree           = cKDTree(scalp_coords)

        # 2. Rim electrode sequence snapped to scalp
        master_sequence  = ["T7", "FT7", "F7", "AF7", "Fp1", "Fpz",
                            "Fp2", "AF8", "F8", "FT8", "T8"]
        electrode_lookup = {str(e[0]).strip().lower(): np.array(e[1:4])
                            for e in cap["electrodes"]}

        rim_pts = []
        for name in master_sequence:
            if name.lower() in electrode_lookup:
                _, idx = tree.query(electrode_lookup[name.lower()])
                rim_pts.append(scalp_coords[idx])
        rim_pts = np.array(rim_pts)

        # 3. Back-anchors on the Y-plane
        _, l_idx = tree.query([rim_pts[0,  0], plane_y, rim_pts[0,  2]])
        _, r_idx = tree.query([rim_pts[-1, 0], plane_y, rim_pts[-1, 2]])
        li_coord, ri_coord = scalp_coords[l_idx], scalp_coords[r_idx]

        # 4. Dense boundary fence - rim arc only (no back anchors for retreat)
        rim_boundary_pts = []
        rim_boundary_pts.extend(self._get_projected_path_internal(
            tree, scalp_coords, li_coord, rim_pts[0]))
        for i in range(len(rim_pts) - 1):
            rim_boundary_pts.extend(self._get_projected_path_internal(
                tree, scalp_coords, rim_pts[i], rim_pts[i + 1]))
        rim_boundary_pts.extend(self._get_projected_path_internal(
            tree, scalp_coords, rim_pts[-1], ri_coord))
        rim_boundary_pts = np.array(rim_boundary_pts)
        rim_tree = cKDTree(rim_boundary_pts)

        # 5. Filter by ELEMENT CENTROID
        all_coords = msh.nodes.node_coord
        centroids  = (all_coords[all_tris[:, 0] - 1] +
                      all_coords[all_tris[:, 1] - 1] +
                      all_coords[all_tris[:, 2] - 1]) / 3.0

        # Distance and nearest point on rim arc
        rim_dists, nearest_idxs = rim_tree.query(centroids)
        nearest_rim_pts = rim_boundary_pts[nearest_idxs]

        # Keep triangles sufficiently above their nearest rim point in z
        # retreat_mm=0 keeps full mask, larger values shrink upward
        is_sufficiently_above = centroids[:, 2] >= (nearest_rim_pts[:, 2] + retreat_mm)

        # Posterior bound unchanged
        is_forward = centroids[:, 1] >= plane_y

        tris_in_mask = all_tris[is_sufficiently_above & is_forward]

        # 6. Nodes = all unique vertices of the surviving triangles
    ...
    def _get_projected_path_internal(self, tree, pts, p1, p2, num_samples=50):
        ...
        line_pts = p1 + t * (p2 - p1)
        _, idxs = tree.query(line_pts)
        return pts[np.unique(idxs)]

    def load_eeg_cap(self, csv_path):
        import pandas as pd
        try:
            df = pd.read_csv(csv_path, header=None, comment="#")
            electrodes, fiducials = [], []
            for _, row in df.iterrows():
                row = row.dropna()
                if len(row) < 5: continue
                rtype, x, y, z, label = str(row.iloc[0]).lower(), float(row.iloc[1]), float(row.iloc[2]), float(row.iloc[3]), str(row.iloc[4]).strip()
                if "fiducial" in rtype: fiducials.append((label, x, y, z))
                elif "electrode" in rtype: electrodes.append((label, x, y, z))
    ...
    def calc_matsimnibs_modified(self, msh, cap=None, msh_surf=None, node_list=None, mask_retreat_mm = 0.0, orientation_mode = 'surface_normal'):
        ...
        if self.matsimnibs_is_defined():
            ...
            # --- CUSTOM MASK LOGIC ---
            # 1. Load the EEG Cap data from your CSV
            # Assuming CSV_FILE is defined globally or passed in
            electrodes, fiducials = self.load_eeg_cap(cap) 
            cap_dict = {"electrodes": electrodes, "fiducials": fiducials}
            ...
            # 3. Call the internal SimNIBS math with your node_list restriction
            # Note: msh.calc_matsimnibs is an internal SimNIBS method
            self.matsimnibs = msh.calc_matsimnibs_modified(
                self.centre, self.pos_ydir, self.distance, 
                msh_surf=msh_surf, node_list=node_list, orientation_mode = orientation_mode
            )
