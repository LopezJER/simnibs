...
class Msh:
   ...
   """
    ...
    def calc_matsimnibs(self, center, pos_ydir, distance, skin_surface=None,
                        msh_surf=None):
        ...
        normal_ray_projection: bool, optional
            If True, finds the scalp point by shooting a ray from the center
            outward along the radial direction (head centre of mass → center),
            rather than using the nearest Euclidean node. More accurate for
            inferior or curved targets such as OFC. Default: False.
        ...
        closest = np.argmin(np.linalg.norm(
            msh_skin.nodes.node_coord - center, axis=1))
    ...
    def get_initial_y_axis(self, pos_ydir, center):
        y = pos_ydir - center
        if np.isclose(np.linalg.norm(y), 0.):
            raise ValueError('The coil Y axis reference is too close to the coil center!')
        y /= np.linalg.norm(y)
    ...
    def calc_matsimnibs_modified(self, center, pos_ydir, distance, skin_surface=None,
                                  msh_surf=None, node_list=None,
                                  orientation_mode='target_aligned'):
        """ Calculate the matsimnibs matrix for TMS simulations
        ...
        msh_surf: simnibs.msh.Msh, optional
            Surface cropped mesh. If not provided, this is computed from self.
        node_list: array of ints, optional
            1-indexed SimNIBS node IDs from the full mesh to restrict the scalp search to.
        orientation_mode: str, optional
            'surface_normal' aligns coil flat against the scalp.
            'target_aligned' points the coil directly at the brain target.
        ...
        # Use full mesh node coords and normals directly
        # node_list contains 1-indexed full mesh node IDs so we index directly
        skin_coords  = self.nodes.node_coord
        skin_normals = self.nodes_normals().value

        if node_list is not None:
            # node_list are 1-indexed full mesh IDs, convert to 0-indexed
            valid_idx     = node_list - 1
            valid_coords  = skin_coords[valid_idx]
            valid_normals = skin_normals[valid_idx]
            print(f"[debug] node_list size: {len(node_list)}")
            print(f"[debug] valid_coords z range: {valid_coords[:, 2].min():.1f} to {valid_coords[:, 2].max():.1f}")
        else:
            # fall back to all tag 1005 scalp nodes
            scalp_tri_mask = (self.elm.elm_type == 2) & (self.elm.tag1 == 1005)
            scalp_node_ids = np.unique(self.elm.node_number_list[scalp_tri_mask, :3]) - 1
            valid_coords   = skin_coords[scalp_node_ids]
            valid_normals  = skin_normals[scalp_node_ids]
            print(f"[debug] no node_list, using all {len(valid_coords)} tag-1005 scalp nodes")

        if len(valid_coords) == 0:
            raise ValueError('No valid scalp nodes found within the node_list mask.')

        closest = np.argmin(np.linalg.norm(valid_coords - center, axis=1))
        center  = valid_coords[closest]
        print(f"[debug] selected scalp point: {center}")

        # Y axis
        y = self.get_initial_y_axis(pos_ydir, center)

        # Normal / Z-axis determination
        if orientation_mode == 'surface_normal':
            normal = valid_normals[closest]
            if np.isclose(np.abs(y.dot(normal)), 1.):
                y = self.get_initial_y_axis(np.array([0., 0., 1.]), center)
                if np.isclose(np.abs(y.dot(normal)), 1.):
                    raise ValueError('The coil Y axis is normal to the surface!')
            z = -normal

        elif orientation_mode == 'target_aligned':
            z = target - center
            z_norm = np.linalg.norm(z)
            if np.isclose(z_norm, 0.):
                raise ValueError('Target and scalp center are the same point.')
            z /= z_norm
            if np.isclose(np.abs(y.dot(z)), 1.):
                y = self.get_initial_y_axis(np.array([0., 0., 1.]), center)
                if np.isclose(np.abs(y.dot(z)), 1.):
                    raise ValueError('The coil Y axis is parallel to the coil normal!')

        else:
            raise ValueError("orientation_mode must be 'surface_normal' or 'target_aligned'")
